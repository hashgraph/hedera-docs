...
```typescript return-bytes-tool-calling-agent.ts
import { Client, PrivateKey, Transaction } from '@hiero-ledger/sdk';

// Set up your operator account and a Hedera client (Testnet by default)
...
// When a transaction-building tool returns bytes, sign and submit them on-chain
