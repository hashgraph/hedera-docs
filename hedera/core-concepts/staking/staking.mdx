...
Nodes have a **minimum stake** and **maximum stake**. The node's minimum stake must be met for staked accounts to be eligible for rewards. Staked HBAR that exceeds the maximum stake does not increase the proportion of rewards returned. The maximum stake for each node is the total number of HBAR divided by the total number of nodes. The minimum stake is 1/4 of that maximum. These values change as more nodes join the network.
