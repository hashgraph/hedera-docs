...
A Hedera network you run on your own machine rather than one of the public networks ([mainnet](#mainnet), [testnet](#testnet), or previewnet). A localnet is ephemeral, auto-funds its own accounts, and has no rate limits or faucet, which makes it suited to fast iteration and CI. [Solo](#solo) is the supported way to run a localnet; the earlier Hiero Local Node tool is deprecated and no longer supported. See [Local Development](/native/local-dev/index) for setup and endpoints.
