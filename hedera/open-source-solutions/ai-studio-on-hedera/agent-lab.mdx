...
Below the center panel is an **Output** terminal showing compilation results and agent runtime logs.
...
The chat interface shows a greeting and a text input at the bottom. Type any natural language request — for example:
...
### Step4: Download and Test Your Agent
...
<Info>
    Mainnet agents spend real HBAR, be cautious when using your agent on Mainnet. To test your agent on Mainnet, you will need to create a wallet and fund the agent account associated with the one you put in the `.env` file.
</Info>