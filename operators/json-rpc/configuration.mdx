...
description: "Environment variables and runtime tuning for the Hiero JSON-RPC Relay."
icon: "sliders"
---
...
| `CHAIN_ID` | yes | Hex-encoded chain ID. `0x127` (mainnet, 295), `0x128` (testnet, 296), `0x12a` (previewnet/local, 298). |
...
The relay ships with three IP-based rate-limit tiers, each tier covering a different cost class of method.
...
| `DEFAULT_RATE_LIMIT` | repo-default | Fallback request limit for methods not assigned a specific tier. |
