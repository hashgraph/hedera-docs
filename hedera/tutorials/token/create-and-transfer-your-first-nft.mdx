...
description: "Using the Hedera Token Service (HTS), you can create non-fungible tokens (NFTs). NFTs are uniquely identifiable. On the Hedera network, the token ID."
---
...
3. [Getting Started: Create a Token](/hedera/getting-started-hedera-native-developers/create-a-token).
...
When creating an NFT, the decimals and initial supply must be set to zero. After creating the token, you mint each NFT using the token mint operation. Specifying a *supply key* during token creation is a requirement to be able to [mint](/hedera/sdks-and-apis/sdks/token-service/mint-a-token) and [burn](/hedera/sdks-and-apis/sdks/token-service/burn-a-token) tokens. The supply key is required to sign mint and burn transactions.
...
Batch minting of 10+ NFTs can run into throughput issues and potentially hit the [transaction limit (throttle cap)](/hedera/networks/mainnet#network-throttles), causing the SDK to throw `BUSY` exceptions. Adding an application-level retry loop can help manage these exceptions for the batch-minting process.
