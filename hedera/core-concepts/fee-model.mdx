...
| `TokenTypes` | Number of token types referenced by the transaction |
...
| `TokenMintNftBase` | Base cost for minting non-fungible tokens |
| `NftUpdate` | Number of NFT updates |
...
| `ConsensusSubmitMessageWithCustomFeeBytes` | Byte count of a message submitted to a topic with custom fees |
| `ConsensusSubmitMessageWithoutCustomFeeBytes` | Byte count of a message submitted to a topic without custom fees |
| `ScheduleCreateContractCallBase` | Base cost for scheduling a contract call |
| `Records` | Number of transaction records produced |
| `StateBytes` | Bytes written to persistent state (e.g., file uploads) |
| `ProcessingBytes` | Bytes processed by the node and network (transaction size on the wire) |
