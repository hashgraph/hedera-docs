...
---
An account alias is a user-friendly identifier that can be classified as either a public key or an [Ethereum Virtual Machine (EVM) address](/learn/core-concepts/accounts/account-properties#evm-address-account-alias). It serves as a reference to the account object, in addition to its account number, and is assigned during the auto account creation process. The purpose of an account alias is to facilitate easier management and recall of accounts, particularly in distributed ledger technology and cryptocurrency contexts where addresses are often complex and difficult to remember. Instead of inputting a long string of characters, users can use simpler and more memorable aliases.

📒 For more information, refer to the [Account Alias section](/learn/core-concepts/accounts/account-properties#account-alias) on the [Account Properties](/learn/core-concepts/accounts/account-properties) page.

### Account ID

---

A unique identifier associated with an account on a distributed ledger. Each account on the ledger has a unique [account ID](/reference/protobuf/basic-types/accountid), which is used to track and manage all transactions associated with that account.
