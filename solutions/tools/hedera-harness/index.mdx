...
- Keep each attempt isolated under `runs/` so you can inspect what the agent produced
...
| **Hedera Harness** | Takes your PRD, asks an agent to build a hedera demo application compatible with scaffold-hbar templates, then validates the result |

Scaffold provisions apps for humans. The harness asks an agent to produce a template from your brief, then checks whether that template works. Skills default to the public [hedera-skills](https://github.com/hedera-dev/hedera-skills) repo; the seed defaults to public [scaffold-hbar](https://github.com/hedera-dev/scaffold-hbar).
