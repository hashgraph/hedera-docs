...
description: "Learn how the Hiero JSON-RPC Relay implements the EVM JSON-RPC standard for Hedera, covering HBAR decimal handling and options from a local Solo network to hosted relays."
---
...
1. [**Solo**](https://solo.hiero.org/docs/)**:** Solo deploys a full Hedera network locally, including a built-in JSON RPC Relay. This option is ideal for quick prototyping, debugging, and isolated testing without external dependencies. It's ideal if you want to run extensive and repeated tests without having to worry about running out of testnet HBAR. See [Using Solo with EVM tools](https://solo.hiero.org/docs/using-solo/using-solo-with-evm-tools/) to connect Hardhat, Foundry, or MetaMask, and [Local Development](/native/local-dev/index) for the local relay endpoint.
...
| Feature                       |          Solo         | Self-hosted RPC Relay |             Third-party RPC Relay            |
| ----------------------------- | :-------------------: | :-------------------: | :------------------------------------------: |
...
| **Localnet**   | `298`    | `http://localhost:37546` (Solo relay)          | Not applicable                          |
