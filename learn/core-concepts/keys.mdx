...
<Warning>
Do not use `setECDSAKeyWithAlias()` if you plan to update or rotate the account key in the future. The EVM Address from Public Key is permanently bound to the original ECDSA public key and does not change when the key is rotated. Use `setKeyWithoutAlias()` instead if key rotation is a requirement. See the [Create an Account](/native/accounts/create) reference for details.
</Warning>
...
<Accordion title="What is an EVM Address from Public Key and why does it matter?">
