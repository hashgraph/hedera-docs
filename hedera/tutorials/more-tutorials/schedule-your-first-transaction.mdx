...
description: "In this tutorial, you'll learn how to create and sign a scheduled transaction. Scheduled Transactions enable multiple parties to easily, inexpensively, and."
---
...
<Card
  ...
  href="/hedera/sdks-and-apis/sdks/schedule-transaction/create-a-schedule-transaction"
/>
...
```javascript JavaScript highlight={2}
//Schedule a transaction
...
The signatures are submitted to the network via a [ScheduleSign](/hedera/sdks-and-apis/sdks/schedule-transaction/sign-a-schedule-transaction) transaction. The [ScheduleSign ](/hedera/sdks-and-apis/sdks/schedule-transaction/sign-a-schedule-transaction)transaction requires the schedule ID of the schedule and the signature of one or more of the required keys. The scheduled transaction has 30 minutes from the time it is scheduled to receive all of its signatures; if the signature requirements are not met, the scheduled transaction will expire.

In the example below, you will submit one signature, confirm the transaction was successful, and get the [schedule info](/hedera/sdks-and-apis/sdks/schedule-transaction/get-schedule-info) to verify the signature was added to the schedule. To verify the signature was added, you can compare the public key of the submitted signature to the public key that is returned from the [schedule info](/hedera/sdks-and-apis/sdks/schedule-transaction/get-schedule-info) request.
...
When the scheduled transaction (transfer transaction) executes a record is produced that contains the transaction details. The scheduled transaction record can be requested immediately after the transaction has executed and includes the corresponding schedule ID. If you do not know when the scheduled transaction will execute, you can always query a [mirror node](/hedera/core-concepts/mirror-nodes/hedera-mirror-node) using the scheduled transaction ID without the `?scheduled` flag to get a copy of the transaction record.
