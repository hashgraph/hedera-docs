...
<Check>
Not sure whether to use the Hiero Local Node, Hashio, or a custom relay setup? Read this [blog post](/hedera/tutorials/local-node/how-to-set-up-a-hedera-local-node) comparing the different options.
</Check>
...
If you’re looking for an end-to-end “Hello, ERC-20” tutorial, see: “[Getting started with Foundry](/hedera/getting-started-evm-developers/deploy-a-smart-contract-with-foundry)” (deploy, interact, verify on Hashscan). This guide focuses on environment configuration and workflow for Localnet/Testnet.
...
1. [Option A: Run Hedera Localnet](#option-a-run-hedera-localnet-hiero-local-node)
2. [Option B: Use Hedera Testnet](#option-b-use-hedera-testnet-hashio)
3. [Step 1: Initialize a Foundry project](#step-1-initialize-a-foundry-project)
4. [Step 2: Add environment variables](#step-2-add-environment-variables)
5. [Step 3: Configure foundry](#step-3-configure-foundry.toml)
6. [Step 4: Add a simple contract](#step-4-add-a-simple-contract)
7. [Step 5: Update the deploy script](#step-5-update-the-deploy-script)
8. [Step 6: Deploy the contract](#step-6-deploy-the-contract)
9. [Step 7: Run tests](#step-7-run-tests)
10. [Step 8: Verifying the contract on Hashscan](#step-8-verifying-the-contract-on-hashscan)
...
1. [**How to Mint and Burn an ERC-721 Token (Part 1)**](/hedera/tutorials/smart-contracts/foundry/how-to-mint-and-burn-an-erc-721-token-using-foundry-part-1)\
   Learn how to create a basic ERC-721 NFT, mint it, and burn it on Hedera.
2. [How to Write Tests in Solidity (Part 2)](/hedera/tutorials/smart-contracts/foundry/how-to-write-tests-in-solidity-part-2)\
   Learn how to start writing tests in Foundry using Solidity
3. [How to Fork the Hedera Network for Local Testing](/hedera/tutorials/smart-contracts/foundry/how-to-fork-the-hedera-network-for-local-testing)\
