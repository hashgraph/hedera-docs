...
| **Accuracy** | May surface outdated or third-party content | Always returns official Hedera docs |

---

## Quick Install (One Command)

For tools that support `npx add-mcp`, you can install the Hedera MCP server across all detected AI agents in a single command:

```bash
npx add-mcp https://docs.hedera.com/mcp
```

This auto-detects which compatible AI tools you have installed (Claude Code, Cursor, VS Code, etc.) and configures them all. You can also target a specific tool:

```bash
# Install only for Cursor
npx add-mcp https://docs.hedera.com/mcp -a cursor

# Install only for Claude Code
npx add-mcp https://docs.hedera.com/mcp -a claude-code

# Install only for VS Code
npx add-mcp https://docs.hedera.com/mcp -a vscode
```

> **Prerequisite:** [Node.js](https://nodejs.org/) (v18+) must be installed for `npx` commands.
