...
<Tip>
**Prefer a local network?** [Solo](https://solo.hiero.org/docs/) is the recommended local development environment, a Kubernetes-native deployment that runs a full Hedera stack (consensus node, mirror node, JSON-RPC relay, and block explorer). The SDK configuration below works against a Solo network; see [Using Solo with Hiero SDKs](https://solo.hiero.org/docs/using-solo/using-solo-with-hiero-sdks/) for the connection details.
</Tip>
