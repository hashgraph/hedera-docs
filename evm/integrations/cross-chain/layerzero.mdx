...
Please note the smallest unit of ***HBAR is the tinybar (8 decimal places)***, while the ***JSON-RPC relay operates 18 decimal places*** for compatibility with EVM tools. This means when dealing with `msg.value`, conversions between tinybars and weibars are necessary. Please take these differences into account, especially when calling `quote`. Reference the [Understanding Hedera's EVM Differences and Compatibility: For EVM Developers](/evm/migration/key-differences-overview) section for a more comprehensive list of differences.
