...
* **A batch may contain no more than one smart contract call (`ContractExecuteTransaction`), and it must be the **final** inner transaction in the batch.
...
<Accordion title="Can I include a smart contract call in a batch transaction?">
Yes, but with restrictions. A batch may contain no more than one smart contract call (`ContractExecuteTransaction`), and it must be the **last** inner transaction in the batch. This ensures the outcome of any preceding inner transactions (such as token transfers or account updates) is finalized before the contract call executes.
</Accordion>
