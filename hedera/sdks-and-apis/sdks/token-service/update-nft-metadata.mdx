...
description: "A TokenUpdateNftsTransaction updates the metadata property of non-fungible tokens (NFTs) on the Hedera network. The transaction requires signing with the."
---
...
<Warning>
🚨 Metadata keys, like other [token keys](/hedera/sdks-and-apis/sdks/token-service/define-a-token#token-keys), must be set during the token creation. If metadata keys are not set when the token is created, they cannot be added later, and you won't be able to update the token's metadata.
</Warning>
...
<table><thead><tr><th>Property</th><th>Description</th></tr></thead><tbody><tr><td><strong>Token ID</strong></td><td>The ID of the NFT to update.</td></tr><tr><td><strong>Serial Numbers</strong></td><td>The list of serial numbers to be updated.</td></tr><tr><td><strong>Metadata</strong></td><td>The new metadata of the NFT(s).</td></tr></tbody></table>
...
<table><thead><tr><th>Method</th><th>Type</th><th>Requirement</th></tr></thead><tbody><tr><td><code>setTokenId(&#x3C;tokenId>)</code></td><td>TokenID</td><td>Required</td></tr><tr><td><code>setSerialNumbers(&#x3C;[int64]>)</code></td><td>List&#x3C;int64></td><td>Required</td></tr><tr><td><code>setMetadata(&#x3C;bytes>)</code></td><td>bytes</td><td>Optional</td></tr></tbody></table>
...
<Accordion title="What is the transaction fee to update a token's metadata?">
The transaction fee to update the metadata of 1 NFT is `$0.001` To update metadata for multiple NFTs in a single call is N x `$0.001` (N being the number of NFTs to update). See the full list of token transaction fees [here](/hedera/networks/mainnet/fees#token-service).
</Accordion>

<Accordion title="What happens if I forget to add metadata keys during token creation?">
...
<Accordion title="Are metadata keys required for all token types?">
...
<Accordion title="Can I still create a token without metadata keys?">
...
<Accordion title="Is it possible to remove metadata keys from a token after it has been created?">
...
<Accordion title="Can the metadata key update the token metadata if the token is paused ?">
No, this is just like a regular [TokenUpdate](/hedera/sdks-and-apis/sdks/token-service/update-a-token). It will fail if token is paused.
</Accordion>

<Accordion title="Can NFT metadata be updated if the asset is held by an account that is frozen for operations with that token?">
