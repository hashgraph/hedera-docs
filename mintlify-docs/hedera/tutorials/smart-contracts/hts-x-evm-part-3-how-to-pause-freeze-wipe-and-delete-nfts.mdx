...
In [HTS x EVM - Part 2](/hedera/tutorials/smart-contracts/hts-x-evm-part-2-kyc-and-update), you learned how to grant / revoke KYC and manage a token using the [Hedera Token Service (HTS) System Smart Contract](/hedera/core-concepts/smart-contracts/system-smart-contracts#hedera-token-service). But those aren't all the token operations you can do!
...
1. [Step 1: Creating, minting, and transferring an NFT](#step-2.-creating-minting-and-transferring-an-nft)
2. [Step 2: Pause a Token](#step-3.-pause-a-token)
3. [Step 3: Unpause a Token](#step-4.-unpause-a-token)
4. [Step 4: Freeze a Token for a Specific Account](#step-5.-freeze-a-token-for-a-specific-account)
5. [Step 5: Unfreeze a Token for a Specific Account](#step-6.-unfreeze-a-token-for-a-specific-account)
6. [Step 6: Wipe a Token](#step-7.-wipe-a-token)
7. [Step 7: Delete a Token](#step-8.-delete-a-token)
8. [Step 8: Deploy Your HTS NFT Smart Contract](#step-8-deploy-your-hts-nft-smart-contract)
...
These steps of the flow have been covered in the [Part 1](/hedera/tutorials/smart-contracts/hts-x-evm-part-1-how-to-mint-nfts#id-2.-minting-an-nft) and [Part 2](/hedera/tutorials/smart-contracts/hts-x-evm-part-2-kyc-and-update). The only difference here is that we set a few different keys to handle pausing, freezing, and wiping.
...
```solidity contracts/MyHTSTokenPFWD.sol wrap
...
* Adding a new function `wipeTokenFromAccount` to wipe the NFT from an account. This effectively burns that token (i.e., reduces the total supply) from a non-treasury account.
  ...
  For [**fungible tokens**](/hedera/support-and-community/glossary#fungible-token), specify an amount to wipe; for [**non-fungible tokens (NFTs)**](/hedera/support-and-community/glossary#non-fungible-token-nft), we specify the serial numbers to be wiped.
* We will also define a new event `TokenWiped`.

#### **Key Code Snippet:**

```solidity contracts/MyHTSTokenPFWD.sol
contract MyHTSTokenPFWD is HederaTokenService, KeyHelper, Ownable {
    ... 
...
```typescript scripts/deployPWD.ts
...
```typescript scripts/mintNFTPFWD.ts
import { network } from "hardhat";
...
```typescript scripts/burnNFTPFWD.ts
import { network } from "hardhat";
...
<Columns cols={2}>
  <Card
    title="Writer: Jake, Developer Relations Engineer"
    arrow
  >
    [GitHub](https://github.com/jaycoolh) | [X](https://x.com/jaycoolh)
  </Card>
  <Card
    title="Editor: Michiel, Developer Relations Engineer"
    arrow
  >
