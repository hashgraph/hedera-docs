...
- **Endpoint:** `<HOSTED_SERVER_URL>/mcp`
- **Transport:** Streamable HTTP
- **Network:** Hedera Testnet

> Replace `<HOSTED_SERVER_URL>` with the official Hashgraph Hosted MCP Server URL once provided.
...
- **Value:** Your Hedera Testnet Account ID (e.g., `0.0.12345`)
...
Replace `0.0.YOUR_ACCOUNT_ID` with your Hedera Testnet account ID. After updating the configuration, restart your client — the Hedera tools should appear in its tool list.
...
Because the hosted server never signs transactions, every state-changing tool returns a response containing the unsigned, hex-encoded transaction bytes. To complete an operation, your client must:
...
Read-only queries — balances, token info, transaction records, exchange rates, and similar — return their results directly. No signing is required for those.
