...
<Note>
  **v4 Breaking Changes (JavaScript SDK):** The package has moved to the `@hashgraph` scope, framework integrations are now separate packages, and plugins must be explicitly imported from `@hashgraph/hedera-agent-kit/plugins`. See the [JavaScript Quickstart](/hedera/open-source-solutions/ai-studio-on-hedera/hedera-ai-agent-kit/hedera-agent-kit-js/quickstart) and [Plugins](/hedera/open-source-solutions/ai-studio-on-hedera/hedera-ai-agent-kit/plugins) pages for updated setup instructions and check out the [migration guide](https://github.com/hashgraph/hedera-agent-kit-js/blob/main/docs/MIGRATION-v4.md) on Github.
</Note>
