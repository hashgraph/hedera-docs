---
title: "Auto-Create Hedera Accounts with HBAR and Token Transfers"
sidebarTitle: "Auto-Create Accounts"
description: "Walkthrough of HIP-32 and HIP-542 auto-account creation on Hedera: send HBAR or HTS tokens to an EVM Address from Public Key to create the account on first transfer."
---

<Warning>
The `AccountBalanceQuery` shown in this tutorial is deprecated and will stop working per the [deprecation timeline](/native/accounts/get-balance). The balance checks below still function today. For new code, use `MirrorNodeAccountBalanceQuery` (JavaScript, available now) or the [Mirror Node REST API](/reference/rest-api/balances). `MirrorNodeAccountBalanceQuery` returns HBAR balances only; for the token balances checked in this tutorial, use the [Mirror Node REST API token balances endpoint](/reference/rest-api/tokens).
</Warning>

[HIP-32](https://hips.hedera.com/hip/hip-32) introduced the ability to auto-create accounts when sending HBAR to an [alias](/native/accounts/create#create-an-account-via-an-account-alias) that does not exist on the network. When HBAR is sent to an alias that does not exist on the network, the account creation fee is deducted from the HBAR sent and the account is auto-created. The new account's initial balance is (sent HBAR - account creation fee). This new method of account creation allowed wallet providers to create free "accounts" to users. However, if a user sends fungible tokens or NFT's to an alias, it would result in an _INVALID_ACCOUNT_ID_ error because the alias does not exist on the network. The auto-account creation flow could not deduct the account creation fee from an HTS token; the account creation fee must be paid in HBAR.
...
<Frame>
  <img src="/images/tutorials/more-tutorials/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers-1.png" />
</Frame>

In this tutorial, a treasury account transfers HTS tokens to Bob's **EVM Address from Public Key** (derived from an ECDSA key) to trigger auto account creation. The flow covers creating fungible tokens and an NFT collection (1000 FT / 5 NFT), deriving Bob's EVM address, and returning Bob's new account ID after the transfer of 10 FT and 1 NFT completes.

---

## Prerequisites

- Get a [Hedera Testnet](https://portal.hedera.com/register) account [here](/native/tutorials/getting-started/create-fund-account).
- Set up your environment and create a client here.

<Accordion title="Get the example code on GitHub:">
  * [auto-create account by sending
  FT](https://github.com/a-ridley/hedera-auto-account-creation-with-ft) *
  [auto-create account by sending
  NFT](https://github.com/a-ridley/hedera-auto-create-account-with-nft)
</Accordion>
...
1. [Create Treasury Account](#create-treasury-account)
2. [Create FTs and NFT Collection](#create-fts-and-an-nft-collection)
3. [Derive Bob's EVM Address from Public Key](#derive-bobs-evm-address-from-public-key)
4. [Return New Account ID](#return-new-account-id)
...
```javascript Create Account With Initial Balance
...
```javascript Create Simple Fungible Token
...
```javascript Create Simple Non-Fungible Token
...
```javascript Mint Token And Create NFT Collection
...
Leverage the **_createFungibleToken_** helper function defined above to create 10000 "Hip-542 example" fungible tokens. Use the code tab switch on the upper left of the code block to see how we use **_createNewNftCollection_** to create our new NFT collection consisting of 5 NFTs.

<CodeGroup>

```javascript CreateFungibleToken
...
```javascript CreateNewNftCollection
// IPFS content identifiers for the NFT metadata
const metadataIPFSUrls: Buffer[] = [
  Buffer.from(
    "ipfs://bafkreiap62fsqxmo4hy45bmwiqolqqtkhtehghqauixvv5mcq7uofdpvt4"
  ),
  Buffer.from(
    "ipfs://bafkreibvluvlf36lilrqoaum54ga3nlumms34m4kab2x67f5piofmo5fsa"
  ),
  Buffer.from(
    "ipfs://bafkreidrqy67amvygjnvgr2mgdgqg2alaowoy34ljubot6qwf6bcf4yma4"
  ),
  Buffer.from(
    "ipfs://bafkreicoorrcx3d4foreggz72aedxhosuk3cjgumglstokuhw2cmz22n7u"
  ),
  Buffer.from(
    "ipfs://bafkreidv7k5vfn6gnj5mhahnrvhxep4okw75dwbt6o4r3rhe3ktraddf5a"
  ),
];
 ...
 */
const nftCreateTxnResponse = await createNewNftCollection(
  client,
...
<a id="create-bobs-ecdsa-public-key-alias"></a>
## Derive Bob's EVM Address from Public Key

The alias is Bob's **EVM Address from Public Key**, the rightmost 20 bytes of the Keccak-256 hash of his ECDSA public key. Transferring value to this address triggers auto account creation. On mainnet, the alias account ID is `0.0.<evmAddress>`.

To learn more about accounts created via an account alias go [here](/native/accounts/create).
...
<Frame>
  <img src="/images/tutorials/more-tutorials/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers-2.png" />
</Frame>

#### Set up helper functions for transferring HTS tokens

Once we have our treasury account with FT and a new NFT collection created, our next step is to transfer them to Bob using their alias. We'll create the **_sendToken_** helper function to send fungible tokens and create **_transferNft_** to send a single NFT.

A quick reminder to use the tab on the left of the code block to switch between the two helper functions.

<CodeGroup>

```javascript SendToken
...
Transfer 5 fungible tokens to Bob using their alias and the helper function **_sendToken_**.

Transfer the NFT with serial number 1 to Bob using the helper function **_transfertNFT_**.

<CodeGroup>

```javascript Send Bob 10 FT
await sendToken(
  client,
  tokenId,
  treasuryAccId,
...
```javascript
export const getAccountIdByAlias = async (
  client: Client,
  aliasAccountId: AccountId
) => {
...
<Frame>
  <img src="/images/tutorials/more-tutorials/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers-3.png" />
</Frame>
...
const tokenBalanceAccountId = accountBalances.tokens._map.get(
  tokenId.toString()
);

if (!tokenBalanceAccountId) {
  throw new Error(
    `account balance does not have tokens for token id: ${tokenId}.`
  );
}
...
tokenBalanceAccountId.toInt() === 5
  ...
  : console.log("Creating account with HTS using EVM address failed");

client.close();
```

<Frame>
  <img src="/images/tutorials/more-tutorials/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers-4.png" />
</Frame>
...
```javascript
export const getNftOwnerByNftId = async (
  client: Client,
  nftTokenId: TokenId,
  exampleNftId: number
) => {
  ...
  if (nftInfo === null) {
    throw new Error("nftInfo is null.");
  }
  const nftOwnerAccountId = nftInfo[0].accountId.toString();
  console.log(
    `- Current owner account id: ${nftOwnerAccountId} for NFT with serial number: ${exampleNftId}`
  );
  return nftOwnerAccountId;
};
```

Then call **_getNftOwnerByNft_** and do a simple check to ensure the account id returned matches the account id created when we sent the NFT to Bob's alias.

```javascript
const nftOwnerAccountId = await getNftOwnerByNftId(
  client,
  nftTokenId,
  exampleNftId
);
...
<Frame>
  <img src="/images/tutorials/more-tutorials/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers-5.png" />
</Frame>

#### Console Output ✅

<Frame>
  <img src="/images/tutorials/more-tutorials/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers/how-to-auto-create-hedera-accounts-with-hbar-and-token-transfers-6.png" />
</Frame>
...
<Columns cols={2}>
  <Card title="Writer: Abi, DevRel Engineer" arrow>
    [GitHub](https://github.com/a-ridley) |
    [LinkedIn](https://www.linkedin.com/in/a-ridley/)
  </Card>
