...
response, err := transaction.NewAccountCreateTransaction().
...
```go Go
hook1 := hook.NewHookCreationDetails().
    SetExtensionPoint(hook.AccountAllowanceHook).
    SetHookID(1).
    SetEvmHook(hook.NewEvmHook().SetContractID(contractId)).
    SetAdminKey(adminKey.PublicKey())

hook2 := hook.NewHookCreationDetails().
    SetExtensionPoint(hook.AccountAllowanceHook).
    SetHookID(2).
    SetEvmHook(hook.NewEvmHook().SetContractID(contractId)).
    SetAdminKey(adminKey.PublicKey())

response, err := transaction.NewAccountUpdateTransaction().
    SetAccountID(accountID).
    AddHookToCreate(hook1).
    AddHookToCreate(hook2).
    SetMaxTransactionFee(hedera.NewHbar(10)).
...
```javascript JavaScript
import { AccountUpdateTransaction, Long } from "@hashgraph/sdk";
