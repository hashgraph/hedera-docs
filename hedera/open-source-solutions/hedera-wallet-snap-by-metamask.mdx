...
description: "MetaMask is a popular Ethereum wallet and browser extension that developers can integrate into a variety of third-party applications."
...
<Columns cols={2}>
  <Card
    title="➡ Hedera Wallet Snap Documentation by Tuum Tech"
    href="https://docs.tuum.tech/hedera-wallet-snap/basics/introduction"
    arrow
  />
  <Card
    title="➡ Hedera Wallet Snap Tutorial FAQs"
    href="/hedera/open-source-solutions/hedera-wallet-snap-by-metamask/metamask-hedera-wallet-snap-tutorial"
    arrow
  />
</Columns>
...
<Accordion title="What are the limitations for connecting MetaMask to the RPC vs the Snap?">
...
<Accordion title="How can I deploy a smart contract on Hedera using MetaMask?">
To deploy a smart contract on Hedera using MetaMask, you will need to use the [Hedera JSON RPC relay](/hedera/core-concepts/smart-contracts/json-rpc-relay). You can deploy using tools compatible with EVM-based chains. For detailed steps, refer to Deploying Smart Contracts on Hedera.
</Accordion>

<Accordion title="Can a signer created via ED25519 account be used for Ethereum-based transactions?">
...
<Accordion title="How can I delegate the signing process to MetaMask or WalletConnect when using Hedera SDK?">
...
<Accordion title="Can I use JSON RPC relay for transactions against Hedera native services?">
...
<Accordion title="Why should I use the Snap instead of adding the Hashio RPC info to MetaMask?">
...
<Accordion title="Is the Snap more suited for custom Hedera functionalities like HCS and HFS, which can’t be done solely with MetaMask?">
