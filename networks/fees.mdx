...
The Hedera network fees tables found below offer a low-end estimate of transaction and query fees for all network services. The tables below contain USD, HBAR, and Tinybar (tℏ) values per each API call. The fee schedule is identical across mainnet and testnet; the only difference is that mainnet fees are paid in HBAR while testnet fees are paid in test HBAR, which is freely available and only useful for development purposes.
...
`high_volume` flag on a [supported entity creation transaction](/learn/core-concepts/high-volume-entity-creation#supported-transaction-types),
...
<Warning>
    ...
    [fee estimation endpoint](/reference/rest-api/network) and set `maxTransactionFee`
...
| Jumbo Transaction (HIP-1086) for Ethereum transactions | [based on gas](/evm/development/gas-fees) |
