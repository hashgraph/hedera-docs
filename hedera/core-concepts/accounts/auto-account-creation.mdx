...
Auto account creation is a unique flow in which applications, like wallets and exchanges, can create free user "accounts" instantly, even without an internet connection. Applications can do this by generating an **account alias.** The alias account ID format used to specify the account alias in Hedera transactions is `<shardNum>.<realmNum>.<alias>`. This is an alternative account identifier compared to the standard account number format `<shardId>.<realmId>.<accountNum>`.

The account alias can be one of the following types:
...
<Accordion title="Public Key">
  ...
  **Example**
  ...
  ECDSA secp256k1 Public Key: `02d588ec1000770949ab77516c77ee729774de1c8fe058cab6d64f1b12ffc8ff07`

  DER Encoded ECDSA secp256k1 Public Key Alias: `302d300706052b8104000a03220002d588ec1000770949ab77516c77ee729774de1c8fe058cab6d64f1b12ffc8ff07`

  Account ID with Public Key Alias: `0.0.302d300706052b8104000a03220002d588ec1000770949ab77516c77ee729774de1c8fe058cab6d64f1b12ffc8ff07`
</Accordion>

<Accordion title="EVM Address">
  An EVM address can be used as an account alias. The EVM address is the rightmost 20 bytes of the 32-byte `Keccak-256` hash of an `ECDSA secp256k1` public key. This calculation is described in the [Ethereum Yellow Paper](https://ethereum.github.io/yellowpaper/paper.pdf).

  The acceptable format for Hedera transactions is the Account ID with the EVM address as the alias. The acceptable format for Ethereum public addresses to denote an account address is the hex-encoded public address.
...
Create an account alias and convert it to the alias account ID format. The alias account ID format requires appending the shard and realm numbers to the account alias. This form of account is purely a local account, i.e., not registered with the Hedera network.

### **2. Deposit tokens to the account alias**

Once the account alias exists, create a **TransferTransaction** that sends HBAR or HTS tokens to that alias. Sending tokens triggers auto account creation on the network, and the first token transferred is automatically associated with the new account.
...
- Specify the account alias in an `AccountInfoQuery` transaction request. The response will return the account's account number.
...
When the alias is an EVM address, the network creates a **hollow account**. A hollow account has an account number and an EVM address but no key. It can receive tokens, but it cannot send tokens or modify account properties until it is a complete account.

### Complete a Hollow Account

To complete a hollow account, submit a Hedera transaction that:

1. Makes the hollow account the **fee payer** for a Hedera transaction, and
2. Signs that transaction with the **ECDSA private key** that matches the EVM address.
...
- Build a transaction (for example, a small [`TransferTransaction`](/hedera/sdks-and-apis/sdks/accounts-and-hbar/transfer-cryptocurrency)).
...
- EVM wallets will set the new account as the transaction fee payer when users sign transactions to complete the account. No further action is required as the RPC will set the user's account ID as the fee payer.
...
<Accordion title="Auto-create an account using an EVM address">
