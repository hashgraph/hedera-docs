...
# # ─── configuration ───
# API_URL="https://mainnet.mirrornode.hedera.com/api/v1/network/nodes?file.id=0.0.102&limit=100&order=asc"
# DOC_FILE="networks/mainnet/mainnet-nodes/README.md"

# # markers for the two tables in the doc file
...
# resp=$(curl -sS --fail --max-time 10 "$API_URL")
# all_nodes=$(jq '.nodes' <<<"$resp")

# node_count=$(jq 'length' <<<"$all_nodes")
# echo "ℹ️  found $node_count nodes in the file"
# [ "$node_count" -eq 0 ] && { echo "⚠️  no nodes found; aborting"; exit 1; }

# nodes_json="$all_nodes"

# # ─── build Table A (nodes with node_id ≤ 15) ───
...
# # ─── build Table B (nodes with node_id > 15) ───
...
# # ─── helper function: inject content between markers using sed ───
# inject_table() {
...
# # ─── write tables to temp files and inject ───
# TABLE_A_FILE=$(mktemp)
...
# echo "✅  Updated $DOC_FILE with two distinct tables"
