...
Hardhat uses the dedicated `@hashgraph/hashscan-verify` plugin to interface with the HashScan Smart Contract Verifier API (which is based on the Sourcify standard).

Install the plugin in your project:

```bash
npm install --save-dev @hashgraph/hashscan-verify
```
...
```typescript
import '@hashgraph/hashscan-verify'; // <--- ADD THIS LINE
import type { HardhatUserConfig } from "hardhat/config";

const config: HardhatUserConfig = {
  plugins: [hardhatToolboxMochaEthersPlugin, hashscanVerify], // ADD IT HERE
  solidity: {
    profiles: {
      default: {
        version: "0.8.28",
