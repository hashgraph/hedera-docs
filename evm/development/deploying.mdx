...
description: "Deploy to Hedera via ContractCreate, EthereumTransaction, or eth_sendRawTransaction, and see how the Besu EVM, Pectra hard fork, and opcodes apply."
---
...
The Hedera EVM supports the standard Ethereum precompiles, plus the BLS12-381 set added in Pectra. The table below lists each precompile and the fork that introduced it.

| Address | Precompile | Introduced |
| --- | --- | --- |
| `0x01` | `ecRecover` | Frontier |
| `0x02` | `SHA2-256` | Frontier |
| `0x03` | `RIPEMD-160` | Frontier |
| `0x04` | `identity` | Frontier |
| `0x05` | `modexp` | Byzantium |
| `0x06` | `ecAdd` (alt_bn128) | Byzantium |
| `0x07` | `ecMul` (alt_bn128) | Byzantium |
| `0x08` | `ecPairing` (alt_bn128) | Byzantium |
| `0x09` | `blake2f` | Istanbul |
| `0x0b` to `0x11` | BLS12-381 operations ([EIP-2537](https://eips.ethereum.org/EIPS/eip-2537)) | Pectra |

Address `0x0a` (KZG point evaluation) was introduced in Cancun for [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844) blob verification. Hedera does not support blobs ([HIP-866](https://hips.hedera.com/hip/hip-866)).

Hedera also exposes its own system contracts as precompiles: the Hedera Token Service (HTS) at `0x167`, the Exchange Rate system contract at `0x168`, and the Pseudo-Random Number Generator (PRNG) at `0x169`.
...
| `0x03`  | [EIP-4844](https://eips.ethereum.org/EIPS/eip-4844)   | Blob transactions. **Not supported** on Hedera; Type 3 is rejected (see [HIP-866](https://hips.hedera.com/hip/hip-866)). |
| `0x04`  | [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702)   | Set-code / EOA code delegation. **Not supported** on Hedera; EIP-7702 is deactivated (see [HIP-1340](https://hips.hedera.com/hip/hip-1340)). |

For access-list usage and SDK examples, see [Ethereum transaction](/native/smart-contracts/ethereum-transaction#supported-ethereum-transaction-types).
...
| -------------------------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Pectra** (May 7, 2025)   | [HIP-1341](https://hips.hedera.com/hip/hip-1341) | • [EIP-2537](https://eips.ethereum.org/EIPS/eip-2537): BLS12-381 precompiles (`0x0b` to `0x11`). <br/>• [EIP-7623](https://eips.ethereum.org/EIPS/eip-7623): calldata gas floor. <br/>• [EIP-7702](https://eips.ethereum.org/EIPS/eip-7702) (Type 4 / EOA code delegation): **deactivated on Hedera**. |
| **Cancun** (Mar 13, 2024)  | Mainnet release [`v0.50`](/networks/release-notes/services#v0.50)          | • New opcodes `TLOAD`/`TSTORE` (transient storage), `MCOPY`, `BLOBHASH`, `BLOBBASEFEE`. <br/>• `SELFDESTRUCT` semantics updated per [EIP-6780](https://eips.ethereum.org/EIPS/eip-6780). <br/>• Blob support (EIP-4844) **not** adopted; Type 3 transactions are rejected ([HIP-866](https://hips.hedera.com/hip/hip-866)).                              |
| **Shanghai** (Apr 12, 2023)| Mainnet release [`v0.38`](/networks/release-notes/services#v0.38)                      | • `PUSH0` opcode. <br/>• Updated `INITCODE` cost for contract creation.                                                                                                                                                                                                                                     |
