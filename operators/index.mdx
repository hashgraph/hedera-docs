...
<Tip>
  **Testing operator components locally?** [Solo](https://solo.hiero.org/docs/) runs a full Hedera stack — consensus node, mirror node, JSON-RPC relay, and block explorer — in a single Kubernetes deployment. Use it to validate mirror node configuration, relay setup, and importer behavior against a local network before connecting to testnet or mainnet.
</Tip>
