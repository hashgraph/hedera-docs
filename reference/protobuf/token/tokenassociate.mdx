...
description: "Hedera protobuf reference for the TokenAssociate transaction used to associate one or more HTS tokens with a Hedera account so it can receive and hold them."
---
...
| --------- | ----------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `account` | [AccountID](/reference/protobuf/basic-types/accountid)      | The account to be associated with the provided tokens                                                                                                                                                |
| `tokens`  | repeated [TokenID](/reference/protobuf/basic-types/tokenid) | The tokens to be associated with the provided account. In the case of `NON_FUNGIBLE_UNIQUE` Type, once an account is associated, it can hold any number of NFTs (serial numbers) of that token type. |
