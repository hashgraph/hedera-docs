...
Starting with **v4**, the Hedera Agent Kit is published under the `@hashgraph` scope and plugins are imported from the `@hashgraph/hedera-agent-kit/plugins` subpath. Framework integrations (LangChain, Vercel AI SDK, and MCP) are now separate packages.
