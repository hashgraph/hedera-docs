...
> **Note**: `@langchain/core` and `langchain` are bundled inside `@hashgraph/hedera-agent-kit-langchain`. You do **not** need to install them separately. You only need to install your LLM provider package.
...
async function main() {
  ...
  // Prepare Hedera toolkit — plugins must be passed explicitly; an empty array means no tools
