---
title: "HTS Part 3: Pause, Freeze, Wipe, and Delete NFTs"
description: "In Part 2, you saw how to enable and disable token Know Your Customer (KYC), update token properties (if a token is mutable), and schedule transactions."
---

In [Part 1](/hedera/tutorials/token/hedera-token-service-part-1-how-to-mint-nfts) of the series, you saw how to mint and transfer an NFT using the Hedera Token Service (HTS). In [Part 2](/hedera/tutorials/token/hedera-token-service-part-2-kyc-update-and-scheduled-transactions), you saw how to enable and disable token [Know Your Customer (KYC)](/hedera/support-and-community/glossary#know-your-customer-kyc), update token properties (if a token is mutable), and schedule transactions. In Part 3, you will learn how to use HTS capabilities that help you manage your tokens. Specifically, you will learn how to:
...
<iframe
  width="720"
  height="405"
  src="https://www.youtube.com/embed/8FWcsbm0udI?si=WG19QOVva_FdVn8F"
  title="How to Mint NFTs on Hedera with JavaScript pt. 3"
  frameborder="0"
...
- Set up your environment [here](/hedera/getting-started-hedera-native-developers/quickstart).

✅ _If you want the entire code used for this tutorial, skip to the_ [_Code Check_](#code-check) _section below._

## Pause a Token

The [pause transaction](/hedera/sdks-and-apis/sdks/token-service/pause-a-token) prevents a token from being involved in any kind of operation across all accounts. Specifying a **_\<pauseKey>_** during the creation of a token is a requirement to be able to pause token operations. The code below shows you that this key must sign the pause transaction. Note that you can’t pause a token if it doesn’t have a pause key. Also keep in mind that if this key was not set during token creation, then a token update to add this key is not possible.
...
In our example below, we pause the token, test that by trying a token transfer and checking the token **_pauseStatus_**, and then we [unpause the token](/hedera/sdks-and-apis/sdks/token-service/unpause-a-token) to enable operations again.

```javascript nft-part3.js
...
[Freezing a token](/hedera/sdks-and-apis/sdks/token-service/freeze-an-account) stops "freezes" transfers of that token for a specific account ID. Note that this transaction must be signed by the **_\<freezeKey>_** of the token. Once a freeze executes, the specified account is marked as “Frozen” and will not be able to receive or send tokens unless unfrozen.

In our example below, we first freeze Alice’s account for the token ID we’re working with, test the freeze by trying a token transfer, and then [unfreeze](/hedera/sdks-and-apis/sdks/token-service/unfreeze-an-account) Alice’s account so she can transact the token again.
...
try {
  ...
  console.log(
    `- See: https://hashscan.io/${network}/transaction/${tokenTransferSubmit4.transactionId}`
  );
}
...
console.log(
  `- See: https://hashscan.io/${network}/transaction/${tokenUnfreezeSubmitTx.transactionId}`
);
```
...
[This operation](/hedera/sdks-and-apis/sdks/token-service/wipe-a-token) wipes the provided amount of fungible or non-fungible tokens from the specified account. You see from the code below that this transaction must be signed by the token's **_\<wipeKey>_**.
...
<Frame>
  <img src="/images/tutorials/token/hedera-token-service-part-3-how-to-pause-freeze-wipe-and-delete-nfts/hedera-token-service-part-3-how-to-pause-freeze-wipe-and-delete-nfts-1.webp" />
</Frame>

## Delete a Token

After you [delete a token](/hedera/sdks-and-apis/sdks/token-service/delete-a-token), it’s no longer possible to perform any operations for that token, and transactions resolve to the error `TOKEN_WAS_DELETED`. Note that the token remains in the ledger, and you can still retrieve some information about it.

The delete operation must be signed by the token **_\<adminKey>_**. Remember from [Part 1](/hedera/tutorials/token/hedera-token-service-part-1-how-to-mint-nfts) that if this key is not set during token creation, then the token is immutable and deletion is not possible.
...
console.log(
  `- See: https://hashscan.io/${network}/transaction/${tokenDeleteSubmitTx.transactionId}`
);
...
In this article, you saw key capabilities to help you manage your HTS tokens, including how to: pause, freeze, wipe, and delete tokens. If you haven’t already, check out [Part 1](/hedera/tutorials/token/hedera-token-service-part-1-how-to-mint-nfts) and [Part 2](/hedera/tutorials/token/hedera-token-service-part-2-kyc-update-and-scheduled-transactions) of this tutorial series to see examples of how to do even more with HTS – you will see how to mint NFTs, transfer NFTs, perform token KYC, schedule transactions, and more.

Continue learning more in our [learning center](https://hedera.com/learning/what-is-hedera-hashgraph)!

<Accordion title="Code Check ✅">
```javascript nft-part3.js
console.clear();
...
<Columns cols={2}>
  <Card
    title="Writer: Ed, DevRel Engineer"
    arrow
  >
    [GitHub](https://github.com/ed-marquez) | [LinkedIn](https://www.linkedin.com/in/ed-marquez/)
  </Card>
  <Card
    title="Editor: Krystal, Senior DX Engineer"
    arrow
  >
