...
description: "How Hedera mirror node components fit together: the importer ingests record files, PostgreSQL stores state, and REST, gRPC, and Web3 APIs expose data."
---
...
Default port `5551`. Exposes the standard Hedera mirror node endpoints documented under the [Mirror Node REST API](/reference/rest-api): `GET /api/v1/transactions/{id}`, `/accounts/{id}`, `/tokens/{id}`, `/contracts/{id}`, etc. Read-only, no authentication required, no transaction fees. This is the API most application code talks to.
...
<CardGroup cols={2}>
  <Card title="Mirror Node REST API" icon="code" href="/reference/rest-api">
