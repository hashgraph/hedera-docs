...
Sets, changes, or clears the account's EOA Code Delegation. Pass a raw 20-byte EVM address to point the account at a delegate contract; pass `0x0000000000000000000000000000000000000000` to clear an existing delegation. After update, any EVM call to the account's address executes the delegate's code in the account's storage context.

For ECDSA accounts with a public-key-derived alias, you can alternatively configure delegation via an EIP-7702 Type 4 Ethereum transaction. ED25519 accounts and ECDSA accounts with a long-zero alias must use this HAPI path. See [EOA Code Delegation](/evm/development/eoa-code-delegation).
</Info>

#### Example: set a delegation target

```javascript JavaScript
import { AccountUpdateTransaction, AccountId, EvmAddress } from "@hashgraph/sdk";

const tx = await new AccountUpdateTransaction()
  .setAccountId(AccountId.fromString("0.0.4567"))
  .setDelegationAddress(EvmAddress.fromString("019b8ee526333d9cbbbc35fff0309794dfa73451"))
  .freezeWith(client)
  .sign(accountKey);
...
const tx = await new AccountUpdateTransaction()
  .setAccountId(AccountId.fromString("0.0.4567"))
  .setDelegationAddress(EvmAddress.fromString("0000000000000000000000000000000000000000"))
  .freezeWith(client)
  .sign(accountKey);
