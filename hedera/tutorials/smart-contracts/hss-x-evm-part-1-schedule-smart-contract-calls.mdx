...
### Traditional EVM Chains (Ethereum, most L2s)

- Smart contracts **cannot schedule** themselves to be called later
- Every contract execution requires an external trigger (EOA or bot)
- Time-based automation needs:
  - Off-chain infrastructure (cron jobs, keepers, bots)
  - Trusted third parties or decentralized keeper networks
  - Additional gas costs for the keeper service

### Hedera with HIP-1215

- Contracts can call the **Schedule Service system contract** (`0x16b`)
- Schedule any contract call to execute at a future time
- The **Hedera consensus layer** stores and executes scheduled transactions
- **Fully on-chain** automation with no external dependencies

This is enabled by two key Hedera Improvement Proposals (HIPs):

- **[HIP-755](https://hips.hedera.com/hip/hip-755)** – Schedule Service System Contract  
  Exposes the Hedera Schedule Service as an EVM-callable system contract at address `0x16b`

- **[HIP-1215](https://hips.hedera.com/hip/hip-1215)** – Generalized Scheduled Contract Calls  
  Allows contracts to schedule **any contract call** via the Schedule Service, including calls to themselves
...
- Some testnet HBAR (get from the [Hedera Portal](https://portal.hedera.com/)).
...
async function main() {
  ...
  // Send 50 HBAR to the contract during deployment
  // This HBAR will be used to pay for scheduled alarm executions
  const HBAR_TO_SEND = "50";
  console.log(
...
AlarmClockSimple contract deployed at: 0x76CFE47809A84e62D633562c350e775e0dB7E1bF
...
Create a script (`setOneShot.ts`) in your `scripts` directory to set a one-shot alarm. Don't forget to replace the `<your-contract-address>` with the address you've just copied.
...
setAlarm tx hash: 0xa94ff67071db3f089dcb81199187c7a42245360658b4e3654a059363a270205e
Alarm ID: 0

Alarm Details:
  User: 0xA98556A4deeB07f21f8a66093989078eF86faa30
  Scheduled time: 2025-12-12T15:37:04.000Z
  Recurring: false
  Times triggered: 0

✅ One-shot alarm scheduled!
⏰ It will automatically trigger in ~10 seconds
📊 View events at: https://hashscan.io/testnet/contract/0x76CFE47809A84e62D633562c350e775e0dB7E1bF/events
```
...
Create a script (`setRecurring.ts`) in your `scripts` directory to set a recurring alarm. Replace `<your-contract-address>` with your contract address.
...
setAlarm tx hash: 0x25b4a292f4e6dde1e760ba57b34631bbdfe0cfbb3b46a211430ac9c967a272a5
Alarm ID: 1

Alarm Details:
  User: 0xA98556A4deeB07f21f8a66093989078eF86faa30
  Scheduled time: 2025-12-12T15:41:59.000Z
  Recurring: true
  Interval: 10 seconds
  Times triggered: 0

Contract HBAR balance: 3.944 HBAR
...
1. **setAlarm transaction**: [View on HashScan](https://hashscan.io/testnet/transaction/1765553815.902461007)

   - This is when you call `setAlarm()`
   - Emits `AlarmScheduled` event

2. **Scheduled triggerAlarm execution**: [View on HashScan](https://hashscan.io/testnet/transaction/1765553824.054298515)
...
When you look at the scheduled transaction on HashScan on the [trace tab](https://hashscan.io/testnet/transaction/1765553824.054298515/trace), you will notice:
...
Contract deployed at: 0xa2Ba0662d646FC337576dFc80058BA3946ab726b
    ✔ should deploy with correct initial state (188ms)
    ✔ should set a one-shot alarm (6759ms)
    ✔ should set a recurring alarm (7424ms)
    ✔ should manually trigger an alarm (13441ms)
    ✔ should not allow non-owner to trigger someone else's alarm (7394ms)
    ✔ should not allow triggering a one-shot alarm twice (15553ms)
